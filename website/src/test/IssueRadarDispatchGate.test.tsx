...
//  * An unresolved readiness lookup never renders as ready. A pending or failed
//    query is not permission to act, the same rule the Investigate control
//    applies to an unresolved record.
...
const DispatchButton = (await import('../apps/issue-radar/components/DispatchButton')).default
...
describe('DispatchButton', () => {
  it('is never clickable in this phase, whatever the repo state', async () => {
    api.getDispatchReadiness.mockResolvedValue(ready())
    mount(<DispatchButton repoRef={REF} />)
    const button = await screen.findByRole('button')
    await waitFor(() => expect(button).toBeDisabled())
  })

  it('tells an unset repo what to set', async () => {
    api.getDispatchReadiness.mockResolvedValue(
      ready({ ready: false, reason: 'no_local_path', local_path: '' }),
    )
    mount(<DispatchButton repoRef={REF} />)
    await waitFor(() =>
      expect(screen.getByRole('button').getAttribute('title')).toMatch(/Point Settings at this repository/i),
    )
  })

  it('distinguishes a broken checkout from an unset one', async () => {
    api.getDispatchReadiness.mockResolvedValue(
      ready({ ready: false, reason: 'checkout_unusable' }),
    )
    mount(<DispatchButton repoRef={REF} />)
    await waitFor(() =>
      expect(screen.getByRole('button').getAttribute('title')).toMatch(/missing, or is no longer a repository/i),
    )
  })

  it('does not claim readiness while the lookup is unresolved', async () => {
    api.getDispatchReadiness.mockRejectedValue(new Error('gateway said no'))
    mount(<DispatchButton repoRef={REF} />)
    await waitFor(() => {
      const title = screen.getByRole('button').getAttribute('title') || ''
      expect(title).not.toMatch(/ready/i)
    })
  })
})
