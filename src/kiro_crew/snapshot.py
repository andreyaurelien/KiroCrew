...
def _do_replace(snap: Path, mc: Path, components: list[str] | None) -> None:
    ...
    try:
        ...
        _restore_everything_from_rollback(backup, mc)
        raise
...
def _restore_everything_from_rollback(backup: Path, mc: Path) -> None:
    """Put every file and tree saved in *backup* back into *mc*.

    The recovery half of replace-mode atomicity. Everything the replace pass is about to
    change is saved under *backup* BEFORE the first mutation, so restoring the whole of
    it returns the data home to one coherent generation regardless of how far the pass
    got. Recovering only the item that failed is what leaves memory half-old and
    half-new.

    Best-effort per entry, and it says so per entry: a recovery that aborts on its first
    problem strands the rest, and by this point the operator's own data is what is at
    stake. Whatever cannot be put back is named so it can be recovered by hand from a
    directory this function never deletes.
    """
    ...
    if failed:
        print("⚠️  Could not put these back: " + ", ".join(failed))
        print(f"   They are still in {backup} — recover them by hand before re-running.")
    else:
